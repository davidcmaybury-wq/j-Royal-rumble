...
const before = await (await fetch(`${U}/api/logs?key=${LOGKEY}`)).json();
...
const after = await (await fetch(`${U}/api/logs?key=${LOGKEY}`)).json();
...
const body = await (await fetch(`${U}/api/logs/${mine.file}?key=${LOGKEY}`)).json();
...
const bad = await fetch(`${U}/api/logs/${encodeURIComponent('../package.json')}?key=${LOGKEY}`);
